...
const PATCH = {
  // Brython's itertools has no pairwise (a 3.10 addition); dim_order's
  // ambiguity check is its only use here. zip(xs, xs[1:]) is pairwise.
  'torch._tensor': (s) => s.replace(
...
add('ctypes', [
  ...
  // Scalar ctypes backed by struct — enough for the byte round-trip that
  // torch.testing's bytes_to_scalar exercises: `ctype.from_buffer((c_byte
  // * sizeof(ctype))(*bytes)).value`. Each scalar carries a struct format
  // and a size; a real sizeof + `cls * n` array + from_buffer close the
  // loop. (The old stub had sizeof()==0, failing 24 storage/dtype tests.)
  'import struct as _ctstruct',
  'class _CDataMeta(type):',
  '    def __mul__(cls, n):',
  '        return _make_array(cls, n)',
  'class _SimpleCData(metaclass=_CDataMeta):',
  '    _fmt_ = None',
  '    _size_ = 0',
  ...
  '        raw = bytes(source._as_bytes()[offset:offset + cls._size_])',
  '        obj = cls()',
  '        if cls._fmt_ is not None:',
  '            obj.value = _ctstruct.unpack("<" + cls._fmt_, raw)[0]',
  '        return obj',
  '    from_buffer_copy = from_buffer',
  ...
  '_CT = {',
  '    "c_bool": ("B", 1), "c_byte": ("b", 1), "c_ubyte": ("B", 1),',
  '    "c_char": ("B", 1), "c_wchar": ("H", 2), "c_short": ("h", 2),',
  '    "c_ushort": ("H", 2), "c_int": ("i", 4), "c_uint": ("I", 4),',
  '    "c_long": ("i", 4), "c_ulong": ("I", 4), "c_longlong": ("q", 8),',
  '    "c_ulonglong": ("Q", 8), "c_size_t": ("I", 4), "c_ssize_t": ("i", 4),',
  '    "c_int8": ("b", 1), "c_uint8": ("B", 1), "c_int16": ("h", 2),',
  '    "c_uint16": ("H", 2), "c_int32": ("i", 4), "c_uint32": ("I", 4),',
  '    "c_int64": ("q", 8), "c_uint64": ("Q", 8), "c_float": ("f", 4),',
  '    "c_double": ("d", 8), "c_void_p": (None, 4), "c_char_p": (None, 4),',
  '    "c_wchar_p": (None, 4), "py_object": (None, 4),',
  '}',
  'for _nm, (_fmt, _sz) in _CT.items():',
  '    globals()[_nm] = _CDataMeta(_nm, (_SimpleCData,), {"_fmt_": _fmt, "_size_": _sz})',
  'class _Ptr: pass',
  'def POINTER(*a, **k):',
  '    return _Ptr',
  'def pointer(*a, **k):',
  ...
  '        return cls._length_ * sizeof(cls._type_)',
  '    return getattr(cls, "_size_", 0)',
  'def addressof(obj):',
  '    return 0',
  'def alignment(obj):',
  '    return 0',
